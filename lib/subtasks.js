...
Subtasks = (function(superClass) {
  ...
  Subtasks.prototype._bind = function() {
    return this.subtasks.on('checked', '.task .simple-checkbox', (function(_this) {
      ...
      return function(e) {
        ...
        e.preventDefault();
        $task = $(e.currentTarget).closest('.task');
        return $task.find('input[type=submit]').click();
      };
    })(this)).on('blur', '.task textarea', this._cancelEdit).on('mousedown touchstart', '.task .link-cancel-edit', this._cancelEdit).on('mousedown touchstart', '.task input[type=submit]', (function(_this) {
      return function(e) {
        return _this._updateTask($(e.currentTarget).closest('.task'));
      };
    })(this)).on('mousedown touchstart', '.task .link-remove-task', (function(_this) {
      return function(e) {
        var $task;
        $task = $(e.currentTarget).closest('.task');
    ...
    })(this)).on('mousedown touchstart', '.task .task-content', (function(_this) {
      return function(e) {
        var $task, $textarea;
        $task = $(e.currentTarget).closest('.task');
  ...
  Subtasks.prototype.addTask = function(task) {
    var $task;
    $task = $(this._taskTpl);
    $task.data('task', task).find('textarea').val(task.desc).end().find('.task-content').text(task.desc);
    if (task.complete) {
