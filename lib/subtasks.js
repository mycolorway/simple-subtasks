...
Subtasks = (function(superClass) {
  ...
  Subtasks.prototype._taskTpl = "<li class='task'><input type='checkbox' /><p class='content'></p><textarea rows='1'></textarea><i class='icon-remove-task'><span>×</span></i></li>";
  ...
  Subtasks.prototype._bind = function() {
    return this.subtasks.on('checked', '.task .simple-checkbox', (function(_this) {
      ...
      return function(e) {
        ...
        if ($textarea.val().trim()) {
          ...
          _this.removeTask($textarea.closest('.task'));
        }
        return $(e.currentTarget).closest('.task').removeClass('editing');
      };
    ...
    })(this)).on('click', '.task p.content', function(e) {
      var $p, $textarea;
      $p = $(e.currentTarget);
      $textarea = $p.next('textarea');
      if (!$textarea.is('[disabled]')) {
        $p.closest('.task').addClass('editing');
        return $textarea.focus();
      }
    });
  };
  ...
  Subtasks.prototype._updateTask = function($textarea) {
    var $p, $task, $tasks, content, descStrs, i, len, newTasks, str, task;
  ...
  Subtasks.prototype.addTask = function(task) {
    var $task;
    $task = $(this._taskTpl);
    $task.data('task', task).find('textarea').val(task.desc).end().find('p.content').text(task.desc);
    if (task.complete) {
